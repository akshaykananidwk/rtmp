...
| `Operation not permitted` / `Input/output error` when a destination starts | FFmpeg reports a blocked firewall, a broken TLS chain and a rejected stream key identically. From 1.9.0 the self-test checks DNS, TCP and TLS separately and says which one stopped it. If it reports the host **is** reachable, the network is fine and the platform is refusing the key — Facebook and Instagram keys from Live Producer are single-use and expire, so fetch a fresh one (or connect the account with OAuth, which fetches a key per broadcast). If it reports the port is unreachable, allow outbound TCP to 1935 and 443 on the VPS firewall and with the hosting provider. |
