...
- **Overlays**: news-channel branding burned into the stream — headline, scrolling ticker, clock/date, logo and colour bars, dragged and resized on a WYSIWYG canvas (with the live picture behind it), stored as percentages so a layout survives a resolution change, and text editable while on air. A font ships with the application, so text works on hosts that confine PHP with open_basedir. Rendered once and copied to every destination, so ten platforms cost one encode.
